        ...
        if env == 'BoxBath':
            ...
            for j in range(time_step_clip):
                ...
                shape_states_ = calc_shape_states_FluidShake(
                    x_box, x_box_last, np.asarray([box_dis_x, box_dis_z]), height, border, z_center)
            ...
            for j in range(time_step):
                ...
                data = [positions[j], velocities[j], clusters, shape_quats[j], scene_params]
...
def make_hierarchy(env, attr, positions, velocities, idx, st, ed, phases_dict, count_nodes, clusters, verbose=0, var=False):
    ...
    for i in range(n_root_level):
        ...
        # [rigid, fluid, wall_0, wall_1, wall_2, wall_3, wall_4, root]
        if env == 'BoxBath':
            attributes[:, 7 + i] = 1
        elif env == 'RiceGrip':
            attributes[:, 1 + i] = 1
...
def prepare_input(data, stat, args, phases_dict, verbose=0, var=False):
    ...
    for i in range(len(instance_idx) - 1):
        ...
        if args.env == 'BoxBath' or args.env == 'FluidShake':
            # [rigid, fluid, wall_0, wall_1, wall_2, wall_3, wall_4, root]
...
class PhysicsFleXDataset(Dataset):
    ...
    def __init__(self, args, phase, phases_dict, verbose):
        ...
        if args.env == 'FluidShake':
            self.data_names = ['positions', 'velocities', 'clusters', 'shape_quats', 'scene_params']
    ...
    def __getitem__(self, idx):
        ...
        return attr, state, relations, n_particles, n_shapes, instance_idx, label, self.phases_dict['material']
